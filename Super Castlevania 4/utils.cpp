...
bool utils::IsOverlapping(const std::vector<Point2f>& polygonL, const std::vector<Point2f>& polygonR)
{
	size_t nrOfVerticesL{ polygonL.size() };
	size_t nrOfVerticesR{ polygonR.size() };
	HitInfo hitInfo{};
	for (size_t i{ 0 }; i < nrOfVerticesL; ++i)
	{
		const Point2f& point1{ polygonL.at(i) };
		const Point2f& point2{ polygonL.at((i + 1) % nrOfVerticesL) };
		if(Raycast(polygonR, point1, point2, hitInfo))
			return true;
	}
	return false;
}

bool utils::IsOverlapping(const std::vector<Point2f>& polygon, const Rectf& rectangle)
{
	size_t nrOfVertices{ polygon.size() };
	float derp{};
	float herp{};
	for (size_t i{ 0 }; i < nrOfVertices; ++i)
	{
		const Point2f& point1{ polygon.at(i) };
		const Point2f& point2{ polygon.at((i + 1) % nrOfVertices) };
		point1.Log();
		point2.Log();
		if (IntersectRectLine(rectangle, point1, point2, derp, herp))
		{
			return true;
		}
...
Point2f utils::GetRectCentreBottom(const Rectf& rect)
{
	float x{ rect.left + (rect.width / 2) };
	float y{ rect.bottom };
	return Point2f(x, y);
...
float utils::GetRandomNumber(int lowestValue, int highestValue, bool isInclusive)
{
